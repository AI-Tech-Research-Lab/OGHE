...
def freq_NN(N_SNV, data, labels):
    ...
    N = np.array([582, 1152, 1197, 2286, 4613])
    TR = np.array([127, 260, 273, 530, 1140])
...
def cluster(N_CNV, data):
  ...
  N = np.array([258, 514, 710, 1024, 2031])
  TR = np.array([0.161, 0.0794, 0.100, 0.061, 0.036])
...
def _get_data_dict(N_CNV:int, N_SNV:int, mode) -> dict:
    ...
    if mode == 'NN':
        ...
        for i in range(0, 11):
            file_name = TRAIN_PATH_CNN + 'pat_' + str(i) + '.npy'
