...
WANDB_PROJECT_NAME = 'CNN_KFold_Ale_2024_04_22_OurPreprocessing'
...
def clean_configurations(configs: [dict]):
    ...
    configs = [
        ...
        if config['n_kernels'] * ((config['DataDim_CNN'][0] - config['kernel_size']) // config['stride'] + 1) +
           config['n_kernels'] * ((config['DataDim_CNN'][1] - config['kernel_size']) // config['stride'] + 1) < 16384
    ]
