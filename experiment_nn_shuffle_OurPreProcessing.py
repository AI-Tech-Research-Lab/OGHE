...
WANDB_PROJECT_NAME = 'NN_KFold_Ale_2024_04_06_OurPreprocessing'

configurations = {
    # 'possible_mode': ['CNN'],
    # 'possible_DataDim_CNN': [[256, 256], [512, 512], [1024, 1024], [2048, 2048]],
    'possible_mode': ['CNN'],
    'possible_DataDim_CNN': [[258, 582], [514, 1152], [710, 1197], [1024, 2286], [2031, 4613]],
...
class GenomicCNN(L.LightningModule):
    def __init__(self, config):
        ...
        if config['mode'] == 'NN':
            self.fc1 = nn.Linear((config['DataDim_NN'][0]+config['DataDim_NN'][1]), 64, bias = False)
            self.fc2 = nn.Linear(64, len(CLASS_NAMES))
        else:
            self.fc1 = nn.Linear((config['DataDim_CNN'][0]+config['DataDim_CNN'][1]), 64, bias=False)
            self.fc2 = nn.Linear(64, len(CLASS_NAMES))
...
if __name__ == '__main__':
    ...
    runs = Parallel(n_jobs=args.n_jobs)(delayed(fetch_run_data)(run.id) for run in wandb_runs)
